...
async function notifyEmailVerified(app, userId) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'account.email_verified');
...
async function notifyPasswordChanged(app, userId) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'account.password_changed');
...
async function notifyAccountLocked(app, userId) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'account.locked');
...
async function notifyAccountUnlocked(app, userId) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'account.unlocked');
...
async function notifyRefundSuccess(app, userId, amount, balance) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'wallet.refund_success', {
        amount: fmtAmount,
        balance: fmtBalance,
      });
...
async function notifyLowBalance(app, userId, balance) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'wallet.low_balance', { balance: fmtBalance });
...
async function notifyPaymentFailed(app, userId, amount) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'wallet.payment_failed', { amount: fmtAmount });
...
async function notifyVehicleEntry(app, userId, plate, slot) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'parking.entry', { plate, slot: slot || 'N/A' });
...
async function notifyVehicleExit(app, userId, plate, totalCost) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, 'parking.exit', { plate, totalCost: fmtCost });
...
async function notifyParkingTimeWarning(app, userId, sessionId, minutesLeft) {
  try {
    ...
    if (notification) {
      const io = getIO(app);
      if (io) await emitNotification(io, userId, notification);
      queueNotificationEmail(userId, ruleKey);
...
async function notifySystemMaintenance(app) {
  try {
    ...
    if (result) {
      const io = getIO(app);
      if (io) await broadcastNotification(io, result.notification);
