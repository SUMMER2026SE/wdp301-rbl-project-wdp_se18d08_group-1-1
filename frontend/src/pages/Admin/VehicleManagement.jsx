...
function SegmentControl({ tab, setTab, pendingCount, modelCount }) {
  const options = [
  ...
  return (
    <div className="inline-grid rounded-[16px] border border-white/[0.08] bg-[#111111]/70 p-1 sm:grid-cols-2">
      {options.map((option) => {
...
export default function VehicleManagement() {
  ...
  const summaryMetrics = [
    ...
    { label: 'Approved', value: '—', note: 'Not loaded by current API', icon: CheckCircle2 },
    ...
    <div className={`relative min-h-[calc(100vh-70px)] overflow-auto bg-[#090909] px-4 py-6 text-white sm:px-6 lg:px-8 ${scrollbarClass}`}>
      ...
      <div className="relative mx-auto max-w-[1500px]">
        ...
        >
          ...
          <button
            type="button"
            onClick={() => { loadPending(); loadModels(); }}
          ...
          >
            <RefreshCw size={15} className={pendingLoading || modelsLoading ? 'animate-spin' : ''} />
            Refresh
          </button>
        </motion.header>
        ...
        >
          <SegmentControl tab={tab} setTab={setTab} pendingCount={pending.length} modelCount={models.length} />
          {tab === 'pending' ? (
