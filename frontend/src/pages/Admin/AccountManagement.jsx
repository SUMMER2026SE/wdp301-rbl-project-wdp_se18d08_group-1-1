...
import { Search, Filter, SortDesc, MoreVertical, X, Mail, Phone, Calendar, UserPlus } from 'lucide-react';
...
export default function AccountManagement() {
    ...
    return (
        <div className="flex-1 h-[calc(100vh-70px)] flex flex-col md:flex-row relative bg-black text-[#e5e2e1]">
            ...
            <section className="w-full md:w-[70%] h-full flex flex-col overflow-y-auto p-5 md:p-10 relative z-10">
                {/* Page Header */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                    <div className="glass-panel rounded-xl p-6 glow-hover transition-all">
                        <div className="flex items-center gap-2 mb-2">
                            <p className="text-xs text-[#d1c5ac] uppercase tracking-wider font-semibold">Total Users</p>
                        </div>
                        <p className="text-3xl text-white font-semibold">{loading ? '...' : totalUsers}</p>
                    </div>
                    <div className="glass-panel rounded-xl p-6 glow-hover transition-all">
                        <div className="flex items-center gap-2 mb-2">
                            <p className="text-xs text-[#d1c5ac] uppercase tracking-wider font-semibold">Active Users</p>
                        </div>
                        <p className="text-3xl text-white font-semibold">{loading ? '...' : activeUsers}</p>
                    </div>
                    <div className="glass-panel rounded-xl p-6 glow-hover transition-all">
                        <div className="flex items-center gap-2 mb-2">
                            <p className="text-xs text-[#d1c5ac] uppercase tracking-wider font-semibold">Staff</p>
                        </div>
                        <p className="text-3xl text-white font-semibold">{loading ? '...' : staffCount}</p>
                    </div>
                    <div className="glass-panel rounded-xl p-6 glow-hover transition-all">
                        <div className="flex items-center gap-2 mb-2">
                            <p className="text-xs text-[#ffd555] uppercase tracking-wider font-semibold">Blocked</p>
                        </div>
                        <p className="text-3xl text-[#ffd555] font-semibold">{loading ? '...' : blockedCount}</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <div className="flex gap-3 w-full sm:w-auto">
                        <button className="glass-panel px-4 py-2 rounded-lg text-sm flex items-center gap-2 hover:bg-white/5 transition-colors border border-[#2a2a2a] text-[#e5e2e1]">
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px]">
                            <thead>
                                <tr className="border-b border-[#2a2a2a] bg-white/5">
                                    <th className="p-4 text-xs text-[#d1c5ac] uppercase font-semibold">User</th>
                                    <th className="p-4 text-xs text-[#d1c5ac] uppercase font-semibold">Department</th>
                                    <th className="p-4 text-xs text-[#d1c5ac] uppercase font-semibold">Role</th>
                                    <th className="p-4 text-xs text-[#d1c5ac] uppercase font-semibold">Status</th>
                                    <th className="p-4 text-xs text-[#d1c5ac] uppercase font-semibold">Joined</th>
                            ...
                            <tbody className="divide-y divide-[#2a2a2a]">
                                {filteredUsers.length === 0 && !loading && (
                                    <tr>
                                        <td colSpan="6" className="p-8 text-center text-[#d1c5ac]">No users found</td>
                                    </tr>
                                )}
                                {loading && (
                                    <tr>
                                        <td colSpan="6" className="p-8 text-center text-[#d1c5ac]">Loading users...</td>
                                    </tr>
                                )}
                                {filteredUsers.map(u => {
                                        ...
                                        >
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    {u.profile?.avatar ? (
                                                        <img alt={fullName} className="w-10 h-10 rounded-full object-cover border border-[#353534]" src={u.profile.avatar} />
                                                    ) : (
                                                        <div className="w-10 h-10 rounded-full bg-[#1c1b1b] flex items-center justify-center border border-[#353534] font-bold uppercase">
                                                            {fullName.charAt(0)}
                                                        </div>
                                                    )}
                                                    <div>
                                                        <p className="text-sm text-white font-medium group-hover:text-[#ffd555] transition-colors truncate max-w-[150px]">{fullName}</p>
                                                        <p className="text-[13px] text-[#d1c5ac] truncate max-w-[150px]">{u.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4 text-sm text-[#e5e2e1] capitalize">{u.role === 'admin' ? 'Management' : (u.profile?.department || 'Operations')}</td>
                                            <td className="p-4">
                                                <span className={`px-2 py-1 border rounded text-xs capitalize ${isStaff ? 'bg-[#ffd555]/10 border-[#ffd555]/30 text-[#ffd555]' : 'bg-[#1c1b1b] border-[#353534] text-[#e5e2e1]'}`}>{u.role}</span>
                                            </td>
                                            <td className="p-4">
                                                <span className="flex items-center gap-1.5">
                                                    <span className={`w-2 h-2 rounded-full ${u.status ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-[#353534]'}`}></span>
                                                    <span className={`text-[13px] ${u.status ? 'text-[#e5e2e1]' : 'text-[#d1c5ac]'}`}>{u.status ? 'Active' : 'Blocked'}</span>
                                                </span>
                                            </td>
                                            <td className="p-4 text-[13px] text-[#d1c5ac]">{formatDate(u.createdAt)}</td>
                                            <td className="p-4 text-right">
                                                <button className="text-[#d1c5ac] hover:text-white transition-colors p-1"><MoreVertical size={18} /></button>
                                            </td>
                                        </tr>
                                    );
                                })}
            ...
            <section className={`w-full md:w-[30%] h-full bg-[#1c1b1b] border-l border-[#2a2a2a] flex-col overflow-y-auto relative z-20 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] ${selectedUser ? 'flex' : 'hidden'}`}>
                ...
                {/* Actions Footer */}
                {selectedUser && (
                    <div className="mt-auto p-8 pt-4 bg-[#0e0e0e] border-t border-[#2a2a2a] space-y-3 sticky bottom-0">
                        <button className="w-full py-2.5 rounded-lg border border-[#ffd555] text-[#ffd555] text-sm font-semibold hover:bg-[#ffd555] hover:text-black transition-colors">Edit User</button>
                        <div className="grid grid-cols-2 gap-3 mt-2">
                            {selectedUser.status ? (
                                <button onClick={() => handleBlockToggle(selectedUser._id, selectedUser.status)} className="py-2 rounded-lg border border-[#93000a] text-[#ffb4ab] text-xs font-semibold hover:bg-[#93000a]/20 transition-colors">Block Account</button>
                            ) : (
                                <button onClick={() => handleBlockToggle(selectedUser._id, selectedUser.status)} className="py-2 rounded-lg border border-emerald-500/30 text-emerald-400 text-xs font-semibold hover:bg-emerald-500/10 transition-colors">Unblock</button>
                            )}
                        </div>
                    </div>
                )}
